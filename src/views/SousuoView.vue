<template>
  <div class="box-border p-3 bg-gray-100 w-[100vw] flex flex-col h-screen relative">
    <header class="flex h-[5%] justify-between items-center relative">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
        class="absolute left-[12%] text-gray-500"
        @click="searchres"
        >
        <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          d="m21 21l-4.343-4.343m0 0A8 8 0 1 0 5.343 5.343a8 8 0 0 0 11.314 11.314Z" />
      </svg>
      <input type="text" class="rounded-2xl py-[2px] pl-[10%] border-2 w-[80%] text-base text-gray-500"
    ...
    <main class="flex-auto overflow-auto ">
      ...
      <div v-show="historyserarch?.length > 0">
        ...
        <ul class="flex items-center flex-wrap my-2">
          <li class="text-xs rounded-xl px-3 py-[2px] bg-white mx-2 mb-2" v-for="item in historyserarch" :key="item.id">
            {{ item }}
          </li>
        </ul>
      </div>
      ...
      <div>
        ...
        <ul class="flex items-center flex-wrap my-2">
          <li class="text-xs rounded-xl px-3 py-[2px] bg-white mx-2 mb-2" v-for="item in youlikesearch" :key="item.id">
            {{ item }}
          </li>
        </ul>
      </div>
      ...
      <!-- 排行榜 -->
      <div ref="wrappers" class="w-[100%] overflow-hidden">
        <ul class="flex wrapper">
          <li v-for="itm in 6" :key="itm.id"
            class="flex flex-col justify-between w-[70vw] mr-[3vw] shadow rounded-2xl box-border p-3 bg-white">
            <h1 class="flex items-center py-[3vw] border-b mb-2"><span class=" font-bold mr-2">热搜榜</span> <span
                class=" rounded-xl px-[1vw] text-xs bg-gray-200 flex items-center"><svg xmlns="http://www.w3.org/2000/svg"
            ...
            <p class="flex items-center" v-for="(item, index) in paihangArr" :key="item.id">
              <span class="mr-2 font-bold text-center w-6 text-orange-700" v-if="index < 3">{{ index + 1 }}</span>
              <span class="mr-2 font-bold text-center w-6 text-gray-500" v-else>{{ index + 1 }}</span>
              <span class="mr-1">{{ item.searchWord }}</span>
              <img v-if="item.iconUrl" class="h-4" :src="item.iconUrl" alt="">
            </p>
          </li>
        </ul>
      </div>
    ...
    <ul v-show="serarchworlds" class="mask absolute left-0 top-[6%] w-[100%] h-[94%] bg-white box-border pl-4">
      <li v-for="item in serarchResult" :key="item.id" class="flex item-center mt-4">
        ...
        <p>
          <!-- {{ item.keyword }} -->
          <span class="text-gray-500">{{ item.keyword.slice(item.keyword.indexOf(serarchworlds),serarchworlds.length) }}</span>
...
export default {
  ...
  updated() {
    ...
    lis[0].parentNode.style.width = (70 + 3) * lis.length + 'vw';
    ph.refresh();
  },
};
</script>