<template>
  <div
    class="box-border bg-gray-100 p-4 w-[100%] flex flex-col h-screen relative dark:bg-black"
  >
    <header class="flex h-[5%] justify-between items-center relative">
      ...
      <input
        type="text"
        class="rounded-2xl py-[2px] pl-[10%] border-2 w-[80%] text-base text-gray-500 dark:bg-black"
        :placeholder="defaultres"
        v-model="serarchworlds"
      />
    ...
    <main class="flex-auto overflow-auto">
      ...
      <div v-show="historyserarch?.length > 0">
        ...
        <ul class="flex items-center flex-wrap my-2">
          <li
            class="text-xs rounded-xl dark:bg-zinc-600 px-3 py-[2px] bg-white mx-2 mb-2"
            v-for="item in historyserarch"
            :key="item.id"
          >
            {{ item }}
          </li>
      ...
      <div>
        ...
        <ul class="flex items-center flex-wrap my-2">
          <li
            class="text-xs rounded-xl px-3 py-[2px] dark:bg-zinc-600 bg-white mx-2 mb-2"
            v-for="item in youlikesearch"
            :key="item.id"
          >
            {{ item }}
          </li>
        ...
        <ul class="flex wrapper">
          ...
          >
            <h1 class="flex items-center py-[3vw] border-b mb-2">
              <span class=" font-bold mr-2">{{ paihangArr[i - 1].data.playlist.name }}</span>
              <span
                class="dark:bg-zinc-600 rounded-xl px-[1vw] text-xs bg-gray-200 flex items-center"><svg xmlns="http://www.w3.org/2000/svg"
            ...
            <div
              ...
              v-for="(item, index) in paihangArr[i - 1].data.playlist.tracks"
              :key="item.id"
            >
            <span class="mr-2 font-bold text-center w-6 text-orange-700" v-if="index < 3">{{ index + 1 }}</span>
              <span class="mr-2 font-bold text-center w-6 text-gray-500" v-else-if="index<20">{{ index + 1 }}</span>
              <p class="whitespace-nowrap text-sm max-w-[90%] overflow-ellipsis" v-if="index < 20">
                {{ item.name }}
              </p>
            </div>
          </li>
    ...
    <ul
      v-show="serarchworlds"
      class="mask absolute left-0 top-[6%] w-[100%] h-[94%] bg-white box-border pl-4"
    >
      <li
        v-for="item in serarchResult"
        :key="item.id"
        class="flex item-center mt-4"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
