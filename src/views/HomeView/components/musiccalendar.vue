            ...
            <li v-for="(item, index) in musiccalendarArr" :key="index" class="flex dark:border-none border-y justify-between p-4">
                <div class="flex flex-col justify-between">
                    <p class=" text-xs">
                        ...
                        <span class="mr-1 text-zinc-500" v-else>明天</span>
                        {{ today }}
                        <span v-if="index == 0" class=" text-red-500 font-semibold ml-4">{{ item.tag }}</span>
                    </p>
                    <p>{{ item.title }}</p>
                </div>
                <img class="w-[15vw] rounded-lg" :src="item.imgUrl" alt="" />
            </li>
        </ul>
    </div>
</template>
<script>
export default {
    props: ['musiccalendarArr', 'today', 'visible'],
    methods: {
        closeDrawer() {
            this.$emit('update:visible', true);
            this.$emit('update:title', '音乐日历');
        },
    },
}
</script>